import { loadProfileView } from './views/perfil.js';
...
export const API_KEY = '808c0f44efd9afa0e316f4c383a0dc1e';
...
export let LANGUAGE = 'es-ES';
export let REGION   = 'ES';


export const _tmdbCache = new Map();
...
export function t(key) { return (I18N[LANGUAGE] || I18N['es-ES'])[key] || key; }
...
export function sfConfirm({ icon='⚠️', kanji='OK', title='¿Estás seguro?', body='', confirmText='Confirmar', cancelText='Cancelar', variant='danger' } = {}) {
...
export const sectionTitle    = document.getElementById('sectionTitle');
export const sectionCount    = document.getElementById('sectionCount');
export const modal           = document.getElementById('modal');
export const modalBody       = document.getElementById('modalBody');
export const closeModalBtn   = document.querySelector('.close');
export const genresContainer = document.getElementById('genresContainer');
export const heroBanner      = document.getElementById('heroBanner');
export const heroBg          = document.getElementById('heroBg');
export const gridViewBtn     = document.getElementById('gridViewBtn');
export const listViewBtn     = document.getElementById('listViewBtn');
export const filterBar       = document.getElementById('filterBar');
...
export function safeSet(el, prop, val) { if (el) el[prop] = val; }

export let isPremium          = JSON.parse(SFStorage.getItem('sf_premium')) || false;
export let premiumTheme       = SFStorage.getItem('sf_theme') || 'red';
export let currentView        = 'all';
export let currentGenre       = 'popular';
export let currentData        = [];
export let favorites          = JSON.parse(SFStorage.getItem('favorites'))    || [];
export let watchLater         = JSON.parse(SFStorage.getItem('watchLater'))   || [];
export let watchHistory       = JSON.parse(SFStorage.getItem('watchHistory')) || [];
export let userRatings        = JSON.parse(SFStorage.getItem('userRatings'))  || {};
export let userNotes          = JSON.parse(SFStorage.getItem('userNotes'))    || {};
export let genresList         = [];
export let tvGenresList       = [];
export let currentPage        = 1;
export let totalPages         = 1;
export let isLoadingMore      = false;
export let currentMediaType   = 'movie';
export let currentSortBy      = 'popularity.desc';
export let isSearchMode       = false;
export let currentSearchQuery = '';
export let isPersonSearch     = false;
export let listSortBy         = 'default';
export let searchMode         = 'content'; // 'content' | 'person'
export let currentDecade      = ''; // '' | '70s' | '80s' | '90s' | '2000s' | '2010s' | '2020s'
export let customLists        = JSON.parse(SFStorage.getItem('customLists')) || {};

export let advFilters = { ratingMin: 0, runtimeMax: 0, language: '', voteMin: 0 };
export let cinemaMode = { active: false, itemId: null, type: null, serverIndex: 0 };

export let marathonQueue    = JSON.parse(SFStorage.getItem('marathonQueue'))    || [];
export let seriesProgress   = JSON.parse(SFStorage.getItem('seriesProgress'))   || {};
export let userBio          = SFStorage.getItem('sf_bio') || '';
export let profileBg        = SFStorage.getItem('sf_profileBg') || '';
export const AVATAR_ICONS = ['👤','🤖','🎃','👾','👽','🐺','💀','👻','🐶','🐵','🐯','👺','🐸','🕊️','🐭','🐱'];
export let userAvatarIcon   = SFStorage.getItem('sf_avatarIcon') || '';
export let top10List        = JSON.parse(SFStorage.getItem('top10List'))        || [];
export let searchHistory    = JSON.parse(SFStorage.getItem('searchHistory'))    || [];
export let streakData       = JSON.parse(SFStorage.getItem('streakData'))       || { lastWatch: null, streak: 0, longest: 0 };
export let achievements     = JSON.parse(SFStorage.getItem('achievements'))     || {};
export let deferredInstall  = null;

export let currentUid = null;
export let cloudSaveTimer = null;
export let leaderboardUnsubscribe = null;

export function getDefaultAvatarIcon(seed='') {
...
export function applyPremiumTheme(t) {
    if (!isPremium) { t = 'red'; }
    const themes = {
...
export function getUserRating(itemId) { return userRatings[String(itemId)] || 0; }
export function setUserRating(itemId, stars) {
    userRatings[String(itemId)] = stars;
    SFStorage.setItem('userRatings', JSON.stringify(userRatings));
    cloudSave();
}
export function renderStars(itemId, interactive = false) {
...
export function getUserNote(itemId) { return userNotes[String(itemId)] || ''; }
export function setUserNote(itemId, note) {
    if (note.trim()) userNotes[String(itemId)] = note.trim();
    else delete userNotes[String(itemId)];
    SFStorage.setItem('userNotes', JSON.stringify(userNotes));
}

export function addToHistory(item, type) {
    if (!item) return;
...
export function isWatched(itemId) { return watchHistory.some(h => h.id === Number(itemId)); }
...
export function renderListToolbar(view) {
...
export function getTopGenresFromFavorites() {
    const genreCount = {};
...
/* loadRecommendations extracted to views/loadRecommendations.js */
...
export function renderPersonResults(persons, query) {
...
export async function loadPersonMovies(personId, personName) {
...
export async function handleRuleta() {
...
export async function renderComparison(id1, id2) {
...
/* loadUpcoming extracted to views/loadUpcoming.js */
...
export let _globalWatchTimer = null;
...
export function decadeToYear(decade) {
...
export function buildDiscoverURL(type, genreId, page=1) {
...
export function removeLoadMoreSpinner() { document.getElementById('loadMoreSpinner')?.remove(); }
...
export function renderListView(items, view) {
...
export function getListForView(view) {
...
export async function loadPopularTV() {
    showLoading();
    if (moviesGrid) moviesGrid.className = moviesGrid.classList.contains('list-view') ? 'movies-grid list-view' : 'movies-grid';
    const sh = document.querySelector('.section-header'); if (sh) sh.style.display = 'flex';
    try {
...
export async function loadByGenre(genreId) {
    showLoading();
    try {
        const type = currentMediaType;
...
export function setHero(item, type='movie') {
    if (!item) return;
...
export function appendItems(items, type) {
...
export function renderCard(item, i, type) {
...
export let cardDelegationInitialized = false;
export function initCardDelegation() {
...
export function isFavorite(id)   { return favorites.some(f => f.id === Number(id)); }
export function isWatchLater(id) { return watchLater.some(f => f.id === Number(id)); }
...
export function toggleWatchLater(itemId) {
    const item = findItemById(itemId); if (!item) return;
...
export async function openModal(itemId, autoPlay=false, type='movie') {
...
export function buildSimilarHTML(items, type, page, totalPages, itemId) {
...
export function bindSimilarEvents(type, currentItemId) {
...
/* loadProfileView extracted to views/loadProfileView.js */


export function drawGenreChart(topGenres) {
...
export function saveCustomLists() {
    SFStorage.setItem('customLists', JSON.stringify(customLists));
    cloudSave();
}
...
export function renderListsManager() {
...
export function renderListDetail(listId) {
...
export function openAddToListPicker(item, type) {
    const listIds = Object.keys(customLists);
...
export function openShareModal(item, type) {
...
export function downloadShareCard(item, type) {
    const title = item.title || item.name || '';
    const year = (item.release_date||item.first_air_date||'').split('-')[0];
    const rating = item.vote_average?.toFixed(1) || 'N/A';
...
export async function loadDondeVer(itemId, type) {
...
export async function loadPorqueViste(currentItemId, type) {
...
export async function loadMoodRecommendation(moodId, cardEl) {
...
export function enterCinemaMode(item, type, serverIndex, servers) {
...
export function changeCinemaServer(dir) {
...
export function toggleCheatsheet(show) {
...
export function setupCardSwipeGestures() {
...
export function showSwipeIndicator(card, text, dir) {
...
export function saveMarathon() { SFStorage.setItem('marathonQueue', JSON.stringify(marathonQueue)); cloudSave(); }

export function addToMarathon(item, type) {
...
export function updateMarathonPanel() {
...
export function openMarathonManager() {
    document.body.style.overflow = 'hidden';
    modal.style.display = 'block';
    const totalMin = marathonQueue.length * 110;
    const h = Math.floor(totalMin/60), m = totalMin % 60;
...
export function openTop10Modal() {
    document.body.style.overflow = 'hidden';
    modal.style.display = 'block';
    renderTop10();
}
...
export function setupTop10DragDrop() {
...
export function exportTop10Image() {
    const canvas = document.createElement('canvas');
...
export async function loadCollection(collectionId, currentItemId) {
...
export function loadTrivia(item) {
...
export function loadAchievementsView() {
...
export function importFromSyncURL() {
...
export async function loadTVSeasons(item) {
...
export const WEEKLY_CHALLENGES_PREMIUM = [
...
export function getWeeklyProgress(challenge) {
...
export function renderLeaderboardHTML(topUsers, own, user) {
...
export function loadContinuarViendo() {
...
export async function loadRecomendados() {
...
export async function initReviews(itemId, mediaType) {
    const sb = window._sb;
    const { data: { user } } = await sb.auth.getUser();
...
export async function loadReviews(itemId, currentUid) {
    const sb = window._sb;
    const list = document.getElementById('sfReviewsList');
    const countEl = document.getElementById('sfReviewsCount');
    if (!list || !sb) return;
...
export async function loadFriendsTab() {
    const sb = window._sb;
    const { data: { user } } = await sb.auth.getUser();
    if (!user) return;
...
export async function toggleFriend(targetUid, action) {
    const sb = window._sb;
    const { data: { user } } = await sb.auth.getUser();
    if (!user) return;
    try {
...
export async function renderFriendsList(uid) {
    const sb = window._sb;
...
export async function renderFriendsActivity(uid) {
    const sb = window._sb;
...
export async function renderFriendRequests(uid) {
    const sb = window._sb;
...
export async function handleFriendRequest(reqId, fromUid, action) {
    const sb = window._sb;
    const { data: { user } } = await sb.auth.getUser();
    if (!user) return;
    try {
...
export function getUserLevel_fromData(data) {
