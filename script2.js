...
window.API_KEY = '808c0f44efd9afa0e316f4c383a0dc1e';
window.BASE_URL = 'https://api.themoviedb.org/3';
window.IMG_BASE = 'https://image.tmdb.org/t/p/w300';
window.IMG_ORIGINAL = 'https://image.tmdb.org/t/p/original';
window.IMG_PROFILE = 'https://image.tmdb.org/t/p/w185';

window.LANGUAGE = 'es-ES';
window.REGION = 'ES';


window._tmdbCache = new Map();
window.tmdbFetch = async function(url) {
...
window.t = function(key) { return (I18N[LANGUAGE] || I18N['es-ES'])[key] || key; }
...
window.sfConfirm = function({ icon='⚠️', kanji='OK', title='¿Estás seguro?', body='', confirmText='Confirmar', cancelText='Cancelar', variant='danger' } = {}) {
...
window.moviesGrid = document.getElementById('moviesGrid');
...
window.sectionCount = document.getElementById('sectionCount');
window.modal = document.getElementById('modal');
window.modalBody = document.getElementById('modalBody');
window.closeModalBtn = document.querySelector('.close');
window.genresContainer = document.getElementById('genresContainer');
window.heroBanner = document.getElementById('heroBanner');
window.heroBg = document.getElementById('heroBg');
window.gridViewBtn = document.getElementById('gridViewBtn');
window.listViewBtn = document.getElementById('listViewBtn');
window.filterBar = document.getElementById('filterBar');
window.sortSelect = document.getElementById('sortSelect');
window.themeToggleBtn = document.getElementById('themeToggle');

window.safeSet = function(el, prop, val) { if (el) el[prop] = val; }

window.isPremium = JSON.parse(SFStorage.getItem('sf_premium')) || false;
window.premiumTheme = SFStorage.getItem('sf_theme') || 'red';
window.currentView = 'all';
window.currentGenre = 'popular';
window.currentData = [];
window.favorites = JSON.parse(SFStorage.getItem('favorites'))    || [];
window.watchLater = JSON.parse(SFStorage.getItem('watchLater'))   || [];
window.watchHistory = JSON.parse(SFStorage.getItem('watchHistory')) || [];
window.userRatings = JSON.parse(SFStorage.getItem('userRatings'))  || {};
window.userNotes = JSON.parse(SFStorage.getItem('userNotes'))    || {};
window.genresList = [];
window.tvGenresList = [];
...
window.currentSortBy = 'popularity.desc';
window.isSearchMode = false;
window.currentSearchQuery = '';
window.isPersonSearch = false;
window.listSortBy = 'default';
window.searchMode = 'content'; // 'content' | 'person'
window.currentDecade = ''; // '' | '70s' | '80s' | '90s' | '2000s' | '2010s' | '2020s'
window.customLists = JSON.parse(SFStorage.getItem('customLists')) || {};

window.advFilters = { ratingMin: 0, runtimeMax: 0, language: '', voteMin: 0 };
window.cinemaMode = { active: false, itemId: null, type: null, serverIndex: 0 };

window.marathonQueue = JSON.parse(SFStorage.getItem('marathonQueue'))    || [];
window.seriesProgress = JSON.parse(SFStorage.getItem('seriesProgress'))   || {};
window.userBio = SFStorage.getItem('sf_bio') || '';
window.profileBg = SFStorage.getItem('sf_profileBg') || '';
window.AVATAR_ICONS = ['👤','🤖','🎃','👾','👽','🐺','💀','👻','🐶','🐵','🐯','👺','🐸','🕊️','🐭','🐱'];
window.userAvatarIcon = SFStorage.getItem('sf_avatarIcon') || '';
window.top10List = JSON.parse(SFStorage.getItem('top10List'))        || [];
window.searchHistory = JSON.parse(SFStorage.getItem('searchHistory'))    || [];
window.streakData = JSON.parse(SFStorage.getItem('streakData'))       || { lastWatch: null, streak: 0, longest: 0 };
window.achievements = JSON.parse(SFStorage.getItem('achievements'))     || {};
window.deferredInstall = null;

window.currentUid = null;
window.cloudSaveTimer = null;
window.leaderboardUnsubscribe = null;

window.getDefaultAvatarIcon = function(seed='') {
...
window.applyPremiumTheme = function(t) {
    if (!isPremium) { t = 'red'; }
    const themes = {
...
window.getUserRating = function(itemId) { return userRatings[String(itemId)] || 0; }
window.setUserRating = function(itemId, stars) {
    userRatings[String(itemId)] = stars;
    SFStorage.setItem('userRatings', JSON.stringify(userRatings));
    cloudSave();
}
window.renderStars = function(itemId, interactive = false) {
...
window.getUserNote = function(itemId) { return userNotes[String(itemId)] || ''; }
window.setUserNote = function(itemId, note) {
    if (note.trim()) userNotes[String(itemId)] = note.trim();
    else delete userNotes[String(itemId)];
    SFStorage.setItem('userNotes', JSON.stringify(userNotes));
}

window.addToHistory = function(item, type) {
    if (!item) return;
...
window.isWatched = function(itemId) { return watchHistory.some(h => h.id === Number(itemId)); }
...
window.sortList = function(items, sortBy) {
    const copy = [...items];
    switch(sortBy) {
...
window.renderListToolbar = function(view) {
...
window.getTopGenresFromFavorites = function() {
    const genreCount = {};
...
window.searchPerson = async function(query) {
    showLoading();
    heroBanner.classList.remove('visible');
    isPersonSearch = true;
    cleanupExtras();
...
window.renderPersonResults = function(persons, query) {
...
window.loadPersonMovies = async function(personId, personName) {
...
window.handleRuleta = async function() {
...
window.openCompareModal = async function() {
    document.body.style.overflow = 'hidden';
...
window.renderComparison = async function(id1, id2) {
...
window.decadeToYear = function(decade) {
...
window.rebuildGenreButtons = function() {
...
window.selectGenre = function(genreId) {
...
window.buildDiscoverURL = function(type, genreId, page=1) {
...
window.showLoadMoreSpinner = function() {
...
window.removeLoadMoreSpinner = function() { document.getElementById('loadMoreSpinner')?.remove(); }
...
window.renderListView = function(items, view) {
...
window.getListForView = function(view) {
...
window.loadPopularMovies = async function() {
    showLoading();
...
window.loadPopularTV = async function() {
    showLoading();
    if (moviesGrid) moviesGrid.className = moviesGrid.classList.contains('list-view') ? 'movies-grid list-view' : 'movies-grid';
    const sh = document.querySelector('.section-header'); if (sh) sh.style.display = 'flex';
    try {
...
window.loadByGenre = async function(genreId) {
    showLoading();
    try {
        const type = currentMediaType;
...
window.setHero = function(item, type='movie') {
    if (!item) return;
...
window.displayItems = function(items, type) {
...
window.appendItems = function(items, type) {
...
window.renderCard = function(item, i, type) {
    const poster       = item.poster_path ? IMG_BASE+item.poster_path : 'https://placehold.co/300x450/1a1a1a/666?text=Sin+imagen';
...
window.bindCardEvents = function() {  }

window.cardDelegationInitialized = false;
window.initCardDelegation = function() {
...
window.isFavorite = function(id)   { return favorites.some(f => f.id === Number(id)); }
window.isWatchLater = function(id) { return watchLater.some(f => f.id === Number(id)); }
...
window.toggleWatchLater = function(itemId) {
    const item = findItemById(itemId); if (!item) return;
...
window.openModal = async function(itemId, autoPlay=false, type='movie') {
...
window.buildSimilarHTML = function(items, type, page, totalPages, itemId) {
...
window.bindSimilarEvents = function(type, currentItemId) {
...
window.drawGenreChart = function(topGenres) {
...
window.openListsManager = function() {
    document.body.style.overflow = 'hidden';
    modal.style.display = 'block';
    renderListsManager();
}

window.renderListsManager = function() {
...
window.renderListDetail = function(listId) {
...
window.openAddToListPicker = function(item, type) {
    const listIds = Object.keys(customLists);
...
window.openShareModal = function(item, type) {
...
window.downloadShareCard = function(item, type) {
    const title = item.title || item.name || '';
    const year = (item.release_date||item.first_air_date||'').split('-')[0];
    const rating = item.vote_average?.toFixed(1) || 'N/A';
...
window.loadPorqueViste = async function(currentItemId, type) {
...
window.loadMoodRecommendation = async function(moodId, cardEl) {
...
window.enterCinemaMode = function(item, type, serverIndex, servers) {
...
window.exitCinemaMode = function() {
...
window.changeCinemaServer = function(dir) {
...
window.handleKeyboardShortcuts = function(e) {
...
window.setupCardSwipeGestures = function() {
...
window.showSwipeIndicator = function(card, text, dir) {
...
window.saveMarathon = function() { SFStorage.setItem('marathonQueue', JSON.stringify(marathonQueue)); cloudSave(); }

window.addToMarathon = function(item, type) {
...
window.toggleMarathonQueue = function() {
...
window.updateMarathonPanel = function() {
...
window.openMarathonManager = function() {
    document.body.style.overflow = 'hidden';
    modal.style.display = 'block';
    const totalMin = marathonQueue.length * 110;
    const h = Math.floor(totalMin/60), m = totalMin % 60;
...
window.openTop10Modal = function() {
    document.body.style.overflow = 'hidden';
    modal.style.display = 'block';
    renderTop10();
}

window.renderTop10 = function() {
...
window.setupTop10DragDrop = function() {
...
window.exportTop10Image = function() {
    const canvas = document.createElement('canvas');
...
window.loadCollection = async function(collectionId, currentItemId) {
...
window.loadTrivia = function(item) {
...
window.loadAchievementsView = function() {
...
window.importFromSyncURL = function() {
...
window.loadTVSeasons = async function(item) {
...
window.getLevelProgress = function() {
    const n = watchHistory.length;
...
window.WEEKLY_CHALLENGES_PREMIUM = [
...
window.getCurrentWeekChallenge = function() {
...
window.getWeeklyProgress = function(challenge) {
...
window.renderLeaderboardHTML = function(topUsers, own, user) {
...
window.buildAdvancedStats = function() {
...
window.loadContinuarViendo = function() {
...
window.loadRecomendados = async function() {
...
window.initReviews = async function(itemId, mediaType) {
    const sb = window._sb;
    const { data: { user } } = await sb.auth.getUser();
...
window.loadReviews = async function(itemId, currentUid) {
    const sb = window._sb;
    const list = document.getElementById('sfReviewsList');
    const countEl = document.getElementById('sfReviewsCount');
    if (!list || !sb) return;
...
window.loadFriendsTab = async function() {
    const sb = window._sb;
    const { data: { user } } = await sb.auth.getUser();
    if (!user) return;
...
window.toggleFriend = async function(targetUid, action) {
    const sb = window._sb;
    const { data: { user } } = await sb.auth.getUser();
    if (!user) return;
    try {
...
window.renderFriendsList = async function(uid) {
    const sb = window._sb;
...
window.renderFriendsActivity = async function(uid) {
    const sb = window._sb;
...
window.renderFriendRequests = async function(uid) {
    const sb = window._sb;
...
window.handleFriendRequest = async function(reqId, fromUid, action) {
    const sb = window._sb;
    const { data: { user } } = await sb.auth.getUser();
    if (!user) return;
    try {
...
window.getUserLevel_fromData = function(data) {
