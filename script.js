...
async function openModal(itemId, autoPlay=false, type='movie') {
    ...
    try {
        ...
        const [detailRes, creditsRes, similarRes] = await Promise.all([
            fetch(`${BASE_URL}/${ep}/${itemId}?api_key=${API_KEY}&language=${LANGUAGE}&append_to_response=videos,translations`),            fetch(`${BASE_URL}/${ep}/${itemId}/credits?api_key=${API_KEY}&language=${LANGUAGE}`),
            fetch(`${BASE_URL}/${ep}/${itemId}/similar?api_key=${API_KEY}&language=${LANGUAGE}&page=1`)
        ]);
...
async function initReviews(itemId, mediaType) {
    ...
    if (window._fb && !window._fb.getDocs) {
        try {
            const fs = await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js');
            window._fb.getDocs    = fs.getDocs;
            window._fb.collection = fs.collection;
            window._fb.query      = fs.query;
            window._fb.where      = fs.where;
            window._fb.orderBy    = fs.orderBy;
            window._fb.deleteDoc  = fs.deleteDoc;
        } catch(e) { console.warn('Could not extend _fb:', e); }
    }
    const fb   = window._fb;
    ...
    document.getElementById('sfReviewSubmit')?.addEventListener('click', async () => {
        ...
        try {
            ...
            const reviewId = `${itemId}_${user.uid}`;
...
async function loadReviews(itemId, currentUid) {
    ...
    try {
        ...
        if (!getDocs) throw new Error('getDocs not available');

        const q    = query(collection(db, 'reviews'), where('itemId', '==', String(itemId)));
            ...
            return `<div class="sf-review-card ${isOwn ? 'sf-review-own' : ''}">
                <div class="sf-rev-top">
                    ...
                    <div class="sf-rev-meta">
                        <span class="sf-rev-name">${r.userName}${premBadge}</span>
                        <span class="sf-rev-date">${dateStr}</span>
                    </div>
                    ${stars}
                    ${isOwn ? `<button class="sf-rev-delete" data-id="${r._id}" title="Eliminar"><i class="fas fa-trash"></i></button>` : ''}
                </div>
...
async function getFbExtra() {
    if (!window._fb.getDocs) {
        const fs = await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js');
        Object.assign(window._fb, {
            getDocs: fs.getDocs, collection: fs.collection,
            query: fs.query, where: fs.where, orderBy: fs.orderBy,
            deleteDoc: fs.deleteDoc, updateDoc: fs.updateDoc,
        });
    }
    return window._fb;
}
...
async function loadFriendsTab() {
    ...
    await renderFriendsList(user.uid);
    await renderFriendsActivity(user.uid);
    await renderFriendRequests(user.uid);
}
...
async function searchUsers(query) {
    ...
    try {
        ...
        const results = snap.docs
            .map(d => ({ _uid: d.id, ...d.data() }))
            .filter(u => u._uid !== user.uid && (
                (u.displayName || '').toLowerCase().includes(q) ||
                (u.email || '').toLowerCase().includes(q)
            ))
            .slice(0, 8);

        const mySnap = await fb.getDoc(fb.doc(fb.db, 'users', user.uid));
        const myFriends = mySnap.data()?.friends || [];

        // Check outgoing requests
        const outgoingReqs = await fb.getDocs(fb.query(fb.collection(fb.db, 'friendRequests'), fb.where('from', '==', user.uid)));
        const outgoing = outgoingReqs.docs.map(d => d.data().to);
...
async function toggleFriend(targetUid, action) {
    ...
    try {
        if (action === 'add') {
            ...
            const existingReq = await fb.getDoc(fb.doc(fb.db, 'friendRequests', `${user.uid}_${targetUid}`));
            if (existingReq.exists()) {
                showToast('<i class="fas fa-clock"></i> Solicitud ya enviada');
                return;
            }
            // Send friend request
            await fb.setDoc(fb.doc(fb.db, 'friendRequests', `${user.uid}_${targetUid}`), {
                from: user.uid,
                to: targetUid,
                status: 'pending',
            ...
            const myRef  = fb.doc(fb.db, 'users', user.uid);
            const targetRef = fb.doc(fb.db, 'users', targetUid);
            const [mySnap, targetSnap] = await Promise.all([fb.getDoc(myRef), fb.getDoc(targetRef)]);
            let myFriends = mySnap.data()?.friends || [];
            let targetFriends = targetSnap.data()?.friends || [];
            myFriends = myFriends.filter(f => f !== targetUid);
            targetFriends = targetFriends.filter(f => f !== user.uid);
            await Promise.all([
                fb.setDoc(myRef, { friends: myFriends }, { merge: true }),
                fb.setDoc(targetRef, { friends: targetFriends }, { merge: true })
            ]);
...
async function renderFriendsList(uid) {
    ...
    try {
        ...
        const profiles = await Promise.all(
            friends.map(fid => fb.getDoc(fb.doc(fb.db, 'users', fid)).then(d => d.exists() ? { _uid: d.id, ...d.data() } : null))
        );
...
async function renderFriendsActivity(uid) {
    ...
    try {
        ...
        profiles.filter(Boolean).forEach(u => {
            (u.watchHistory||[]).slice(0, 5).forEach(item => {
                events.push({
                    uid:    u._uid,
...
async function renderFriendRequests(uid) {
    ...
    if (!el) return;
    try {
        const reqsSnap = await fb.getDocs(fb.query(fb.collection(fb.db, 'friendRequests'), fb.where('to', '==', uid), fb.where('status', '==', 'pending')));
...
async function handleFriendRequest(reqId, fromUid, action) {
    ...
    try {
        if (action === 'accept') {
            // Add mutual friends
            const myRef = fb.doc(fb.db, 'users', user.uid);
            const fromRef = fb.doc(fb.db, 'users', fromUid);
            const [mySnap, fromSnap] = await Promise.all([fb.getDoc(myRef), fb.getDoc(fromRef)]);
            let myFriends = mySnap.data()?.friends || [];
            let fromFriends = fromSnap.data()?.friends || [];
            if (!myFriends.includes(fromUid)) myFriends.push(fromUid);
            if (!fromFriends.includes(user.uid)) fromFriends.push(user.uid);
            await Promise.all([
                fb.setDoc(myRef, { friends: myFriends }, { merge: true }),
